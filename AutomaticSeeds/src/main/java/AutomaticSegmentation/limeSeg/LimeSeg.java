...
public class LimeSeg implements Command {   
     ...
     public Optimizer opt;                   			// Associated Optimizer, filled with dots currently being optimized
     public JOGL3DCellRenderer jcr;					// Associated 3D Viewer
    
     public ArrayList<Cell> allCells;  				// List of cells currently stored by LimeSeg
     public boolean notifyCellExplorerCellsModif;		// Flags any change of allCells to the cellExplorer tab (Swing GUI)
   
     public ArrayList<DotN> dots_to_overlay;			// Set of dots being overlayed on workingImP
    
     public Cell currentCell;    						// Current selected Cell of limeseg
    
     public DotN currentDot;							// Current selected Dot of limeseg
   
	 public ImagePlus workingImP;            			// Current working image in IJ1 format - Used for IJ1 interaction (ROI, and JOGLCellRenderer synchronization)
	
	 public int currentChannel=1;						// specifies the Channel of the workingImP to work with
    	
	 public int currentFrame=1;						// specifies the Frame of the workingImP to work with
    
    public  ArrayList<DotN> copiedDots;				// Internal equivalent of Clipboard for set of dots in limeseg
     Skeleton2D skeleton;  							// Current skeleton of Limeseg. Serves to generate initial shapes in 3D
    
     public boolean optimizerIsRunning=false;			// Flags if optimizer is running
    public  boolean requestStopOptimisation=false;	// Flags is a stop optimisation command has been requested
    ...
    @DisplayableOutput(pr=6)
     public String getClippedDotsState() {
    	String str="";
    	str+="ClippedDots:\n";
    	if (copiedDots!=null) {
     ...
     */
     public ArrayList<DotN> makeXYSheet(float d_0, float pxi, float pyi, float pxf, float pyf, float pz) {
     ...
     * @return
     */
     public ArrayList<DotN> makeSphere(float d_0, float px, float py, float pz, float radius) {
        ArrayList<DotN> ans = new ArrayList<DotN>();
     ...
     * @return
     */
     public ArrayList<DotN> makeCylinder(float d_0, float px, float py, float pz, float radius, float height) {
        ArrayList<DotN> ans = new ArrayList<DotN>();
     ...
     */
     public boolean notifyCellRendererCellsModif;		// Flags any change of allCells to the 3D Viewer (JOGLCellRenderer)
     ...
     *  a bit ugly though
     */
    public  void requestFillBufferCellRenderer() {
     ...
     * Helper function for 3D viewer
     */
    public  void setBuffFilled(boolean flag) {
     ...
     * Helper function for 3D viewer
     */
    public  void fillBufferCellRenderer() {
        jcr.fillBufferCellRenderer_PC();
        jcr.fillBufferCellRenderer_TR();
    }
    
    public  void fillBufferCellRenderer(ArrayList<DotN> aList) {
     ...
     */
     public void addToOverlay(CellT ct) {
     ...
     */
     public void addToOverlay(Cell c) {
    ...
    public  void setWorkingImage(String imageName) {
    	System.out.println("imageName="+imageName);
   		ImagePlus imtest = WindowManager.getImage(imageName);
   		if (imtest!=workingImP) {
   			workingImP=imtest;
   			this.notifyCellExplorerCellsModif=true;
   			updateWorkingImage();    
   		}
    }
     ...
     * @param frame
     */
     public void setWorkingImage(int imageID, int channel, int frame) {
     ...
     */
     public void setWorkingImage(ImagePlus img, int channel, int frame) {        
     ...
     */
     public void setWorkingImage(RandomAccessibleInterval img, int channel, int frame) {        
        if (!optimizerIsRunning) {
        	opt.setWorkingImage(img, channel, frame);  
        	workingImP = null;
        }
     ...
     */
     public void putCellTo3DDisplay(Cell c) {
        make3DViewVisible();
    ...
    /**
     * Displays a swing table with current Cells and corresponding CellTs present in this
     */
    	...
    	if ((opt!=null)) {
    		...
    		for (CellT ct:ctInOptimizer) {
    			...
    			if (ct.c.cellTs.size()==0) {
    				...
    				if (remove) {this.allCells.remove(c);}
    			}
    		}
    		this.clearOptimizer();    		
    		...
    		this.notifyCellExplorerCellsModif=true;
    		this.notifyCellRendererCellsModif=true;
    	}
    }
    ...
    @IJ1ScriptableMethod(target=OPT, ui="STD", tt="()", pr=6)
     public void putAllCellsToOptimizerRespectChannel() {
    	for (Cell c:allCells) {
    		if ((c.cellChannel==this.currentChannel)&&(c.getCellTAt(this.currentFrame)!=null)) {
    			this.opt.addDots(c.getCellTAt(this.currentFrame));    		
    		}
    	}
    }
    ...
    @IJ1ScriptableMethod(target=STATE, ui="STD", tt="(int cChannel)", pr=1)
     public void setCurrentChannel(int cChannel) {
    ...
    @IJ1ScriptableMethod(target=OPT, ui="STD", tt="(String str) // Format MinX,MaxX,MinY,MaxY,MinZ,MaxZ", pr=4)
     public void setOptBounds(String str) {
    ...
    @IJ1ScriptableMethod(target=OPT, ui="STD", pr=2)
     public void putAllCellTsToOptimizer() {
        for (int i=0;i<allCells.size();i++) {
    ...
    @IJ1ScriptableMethod(target=OPT, ui="STD", tt="(String paramName, double value)", pr=3)
     public void setOptimizerParameter(String paramName, double value) {
    	assert opt!=null;
    ...
    @IJ1ScriptableMethod(target=OPT, tt="(String paramName, Double value)")
     public void getOptimizerParameter(String paramName, Double[] value) {
    	assert opt!=null;
    ...
    @IJ1ScriptableMethod(target=OPT, tt="(int NStep)", ui="STD", pr=-5, newThread=true)
     public int runOptimisation(int NStep) {
    ...
    @IJ1ScriptableMethod(target=VIEW_2D, ui="STD", pr=0)
     public void putCurrentCellToOverlay() { 
    	if (currentCell!=null) {
     ...
     */
    @IJ1ScriptableMethod(target=VIEW_2D, ui="STD", pr=0)
     public void putCurrentSliceToOverlay() { 
    	if (workingImP!=null) {
	    	...
	    	for (Cell c:allCells) {
	    		CellT ct = c.getCellTAt(this.currentFrame);
	    		if (ct!=null) {
	    			for (DotN dn:ct.dots) {
	    				if ((int)(dn.pos.z/ZS)==zSlice-1) {
	    					this.dots_to_overlay.add(dn);
	    				}
	    			}
	    		}
	    	}
    ...
    @IJ1ScriptableMethod(target=VIEW_2D, ui="STD", pr=0)
     public void putCurrentTimePointToOverlay() { 
    	if (workingImP!=null) {
	    	for (Cell c:allCells) {
	    		CellT ct = c.getCellTAt(this.currentFrame);
	    		if (ct!=null) {
    ...
    @IJ1ScriptableMethod(target=VIEW_2D, ui="STD", pr=1)
     public void addAllCellsToOverlay() {
        for (int i=0;i<allCells.size();i++) {
            Cell c= allCells.get(i);
            addToOverlay(c);
        }
     ...
     */
    @IJ1ScriptableMethod(target=VIEW_3D, ui="STD", pr=2)
     public void putCurrentCellTo3DDisplay() {
    	if (jcr!=null) {
     ...
     */
    @IJ1ScriptableMethod(target=VIEW_3D, ui="STD", pr=2)
     public void setDefaultColor3DDisplay() {
        make3DViewVisible();
        this.jcr.colorSupplier = new DefaultDotNColorSupplier();
        notifyCellRendererCellsModif=true;
    }
     ...
     */
    @IJ1ScriptableMethod(target=VIEW_3D, ui="STD", pr=2)
     public void setCurrentCellColorLUT() {
        make3DViewVisible();
        this.jcr.colorSupplier = new CurrentCellColorLUT();
        notifyCellRendererCellsModif=true;
    }
    ...
    @IJ1ScriptableMethod(target=VIEW_3D, ui="STD", tt="(float px,float py, float pz)", pr=6)
     public void set3DViewCenter(float px, float py, float pz) {
    	//make3DViewVisible();
    	if (jcr!=null) {
    		jcr.lookAt.x=px;            
        	jcr.lookAt.y=py;            
    ...
    @IJ1ScriptableMethod(target=VIEW_3D, ui="STD", tt="(float rx,float ry, float rz)", pr=7)
     public void set3DViewRot(float rx, float ry, float rz) {
    	//make3DViewVisible();            
        if (jcr!=null) {
        	jcr.view_rotx=rx;            
        	jcr.view_roty=ry;            
    ...
    @IJ1ScriptableMethod(target=VIEW_3D, ui="STD", pr=1, newThread=true)
     public void putAllCellsTo3DDisplay() {
        //make3DViewVisible();
    ...
    @IJ1ScriptableMethod(target=VIEW_3D, ui="STD", tt="(int vMode)", pr=4)
     public void set3DViewMode(int vMode) {
    	if (jcr!=null) {
    ...
    @IJ1ScriptableMethod(target=VIEW_3D, ui="STD", tt="(float zoom)", pr=5)
     public void set3DViewZoom(float zoom) {
    	if (jcr!=null) {
    ...
    @IJ1ScriptableMethod(target=VIEW_3D+TS+CURRENT_CELL, ui="STD", tt="(int vMode)", pr=8)
	 public void setCell3DDisplayMode(int vMode) {
		if (currentCell!=null) {   
	...
	@IJ1ScriptableMethod(target=STATE, ui="STD", tt="(int index)", pr=3)
	 public void selectCellByNumber(int index) {
    ...
    public  void checkSelectColorLUT() {
        if (jcr!=null) {
    ...
    @IJ1ScriptableMethod(target=CURRENT_CELL, ui="STD", tt="(float r, float g, float b, float a) : Set cell color (parameters are between 0 and 1).", pr=2)
     public void setCellColor(float r, float g, float b, float a) {
    	if (currentCell!=null) {
    ...
    /**
     * !Warning! removes all cells of this
     */
    ...
    /**
     * Clear the cell named by arg from this
     * @param arg
     */
    @IJ1ScriptableMethod(target=STATE, tt="(String arg)")
     public void clearCell(String arg) {        
    ...
    @IJ1ScriptableMethod(target=STATE, ui="STD", tt="(String id)", pr=4)
     public void selectCellById(String id) {
    ...
    @IJ1ScriptableMethod(target=CURRENT_CELLT, tt="", pr=0)
     public void getCellSurface(Double[] value) {
		value[0]=(double)0;
    	if (currentCell!=null) {
     ...
     */
    @IJ1ScriptableMethod(target=CURRENT_CELLT, tt="", pr=0)
     public void getCellVolume(Double[] value) {
		value[0]=(double)0;
    	if (currentCell!=null) {
    		if (currentCell.getCellTAt(currentFrame)!=null) {
    			if (currentCell.getCellTAt(currentFrame).dots!=null) {
    ...
    @IJ1ScriptableMethod(target=CURRENT_CELLT, ui="STD", pr=0)
     public void putCurrentCellTToOptimizer() {
    ...
    @IJ1ScriptableMethod(target=CURRENT_CELLT, ui="STD", pr=3)
     public void clearDotsFromCellT() {
        if (currentCell==null) {
            return;
        }
        CellT ct = currentCell.getCellTAt(currentFrame);
	...
	@IJ1ScriptableMethod(target=CURRENT_DOT, ui="STD", tt="(float nx, float ny, float nz)", pr=2)
	 public void setDotNorm(float nx, float ny, float nz) { 
	    if (currentDot!=null) {
	...
	@IJ1ScriptableMethod(target=CURRENT_DOT, ui="STD", tt="(float px, float py, float pz)", pr=1)
	 public void setDotPos(float px, float py, float pz) { 
	    if (currentDot!=null) {
	        currentDot.pos.x=px;
	        currentDot.pos.y=py;
	        currentDot.pos.z=pz;
	...
	@IJ1ScriptableMethod(target=CURRENT_DOT, ui="STD", tt="(int mov_,int rot_, int des_, int gen)", pr=3)
	 public void setDotProps(int mov_, int rot_, int des_, int gen_) {
        boolean mov=(mov_==1);
        boolean rot=(rot_==1);
        boolean des=(des_==1);
        boolean gen=(gen_==1);
	...
	@IJ1ScriptableMethod(target=CURRENT_DOT, ui="STD", tt="(int flag)", pr=4)
	 public void setDotFlag(int flag) {
	    if (currentDot!=null) {
    ...
    @IJ1ScriptableMethod(target=CLIPPED_DOTS, ui="STD", pr=6)
     public void invertClippedDotsPolarity() {
        if (copiedDots!=null) {
    ...
    @IJ1ScriptableMethod(target=CLIPPED_DOTS, ui="STD", tt="(int mov_,int rot_, int des_, int gen_)", pr=5)
     public void setClippedDotsProps(int mov_, int rot_, int des_, int gen_) {
        boolean mov=(mov_==1);
        boolean rot=(rot_==1);
        boolean des=(des_==1);
        boolean gen=(gen_==1);
    ...
    @IJ1ScriptableMethod(target=CLIPPED_DOTS, ui="STD", tt="(float tx,float ty,float tz)", pr=4)
     public void translateDots(float tx,float ty,float tz) {
        float ZS=(float)opt.getOptParam("ZScale");
        if (copiedDots!=null)
        for (int i=0;i<copiedDots.size();i++) {
                DotN dn=copiedDots.get(i);
     ...
     */
  	 ...
  	 * @param roi
  	 */
  	 public void addRoiToSkeleton(Roi roi) {
  		 if (roi!=null) {
  	 ...
  	 public void addRoiToSkeleton(Roi roi, int zpos) {
 		 if (roi!=null) {
            FloatPolygon pol = roi.getFloatPolygon();
            PolygonSkeleton polSk = new PolygonSkeleton();
            ArrayList<Vector3D> vp = new ArrayList<>();
    ...
    @IJ1ScriptableMethod(target=CLIPPED_DOTS, ui="STD", pr=3)
     public void generateAndCopySkeletonSurface() {
      ...
      @IJ1ScriptableMethod(target=CLIPPED_DOTS, ui="STD", tt="(double px, double py, double pz, double radius)", pr=0)
       public void makeSphere(float px, float py, float pz, float radius) {
      ...
      @IJ1ScriptableMethod(target=CLIPPED_DOTS)
       public void makeXYSheet(float pxi, float pyi, float pxf, float pyf, float pz) {       
    ...
    //@IJ1ScriptableMethod(target=IO, ui="PathWriter", tt="(String path)", pr=1)
     public void saveStateToXmlPlyv1(String path) {
        IOXmlPlyLimeSeg.saveState(new LimeSeg(), "0.1", path);
    }
    @IJ1ScriptableMethod(target=IO, ui="PathWriter", tt="(String path)", pr=1)
     public void saveStateToXmlPly(String path) {
    ...
    @IJ1ScriptableMethod(target=IO, ui="PathOpener", tt="(String path)", pr=0)
     public void loadStateFromXmlPly(String path) {
     ...
     */
    @IJ1ScriptableMethod(target=OPT, tt="(String paramName, Double value)")
     public void getCurrentChannel(Double[] value) {
     ...
     */
    @IJ1ScriptableMethod(target=OPT, tt="(String paramName, Double value)")
     public void getCurrentFrame(Double[] value) {
     ...
     */
    @IJ1ScriptableMethod(target=OPT, tt="(String paramName, Double value)")
     public void getNCells(Double[] value) {
    ...
    @IJ1ScriptableMethod(target=VIEW_3D, tt="(String paramName, Double value)")
     public void get3DViewCenter(Double[] pX, Double[] pY, Double[] pZ) {        
        make3DViewVisible();
    ...
    @IJ1ScriptableMethod(target=CURRENT_CELL, tt="(String paramName, Double value)")
     public void getCellColor(Double[] r, Double[] g, Double[] b, Double[] a) {
	    if (currentCell!=null) {
     ...
     */
    @IJ1ScriptableMethod(target=VIEW_3D, tt="(String paramName, Double value)")
     public void get3DViewMode(Double[] value) {
    ...
    @IJ1ScriptableMethod(target=VIEW_3D,ui="STD", tt="()")
     public void enableTrackCurrentCell() {
        make3DViewVisible();
     ...
     */
    @IJ1ScriptableMethod(target=VIEW_3D,ui="STD", tt="()")
     public void disableTrackCurrentCell() {
        make3DViewVisible();
    ...
    @IJ1ScriptableMethod(target=CURRENT_CELL+TS+VIEW_3D, tt="(int vMode)")
     public void getCell3DDisplayMode(Double[] vMode) {
		if (currentCell!=null) {
     ...
     */
    @IJ1ScriptableMethod(target=CURRENT_CELL, tt="(String paramName, Double value)")
     public void getDotPos(Double[] px, Double[] py, Double[] pz) {
	    if (currentDot!=null) {
     ...
     */
    @IJ1ScriptableMethod(target=CURRENT_CELL, tt="(String paramName, Double value)")
     public void getDotNorm(Double[] nx, Double[] ny, Double[] nz) {
	    if (currentDot!=null) {
    ...
    @IJ1ScriptableMethod(target=CURRENT_CELLT, tt="(Double value)")
     public void getNDots(Double[] value) {
		value[0]=(double)0;
    	if (currentCell!=null) {
    		if (currentCell.getCellTAt(currentFrame)!=null) {
    			if (currentCell.getCellTAt(currentFrame).dots!=null) {
    ...
    @IJ1ScriptableMethod(target=CURRENT_CELLT, tt="(Double[] px, Double[] py, Double[] pz)")
     public void getCellCenter(Double[] px, Double[] py, Double[] pz) {
        px[0] = new Double(0);
        py[0] = new Double(0);
        pz[0] = new Double(0);
    	if (currentCell!=null) {
